...
def generate_time_series(temp_values, batch_size, n_steps):
    ...
    return series[:, :n_steps-1, np.newaxis].astype(np.float32), series[:, -1, np.newaxis].astype(np.float32)
...
if __name__ == "__main__":
    ...
    plot_series(series, y=all_true_labels.reshape(-1, 1), y_pred=all_predictions.reshape(-1, 1))